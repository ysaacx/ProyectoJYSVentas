GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[VENT_PEDIDSS_BusCotizacion]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[VENT_PEDIDSS_BusCotizacion] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[VENT_PEDIDSS_BusCotizacion]
(
	 @FecIni DateTime
	,@FecFin DateTime
...
Begin
	if @Rehusados = 1
		...
		Select * From #Pedidos Where Not PEDID_Estado In  ('R') Order By PEDID_FechaDocumento Desc
End 
Else
Begin
	if @Rehusados = 1
		Select * From #Pedidos Where Not PEDID_Estado In  ('X', 'C') Order By PEDID_FechaDocumento Desc
	else
