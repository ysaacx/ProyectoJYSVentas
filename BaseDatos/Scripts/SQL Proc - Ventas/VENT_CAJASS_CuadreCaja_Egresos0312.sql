GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[VENT_CAJASS_CuadreCaja_Egresos0312]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[VENT_CAJASS_CuadreCaja_Egresos0312] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[VENT_CAJASS_CuadreCaja_Egresos04122014]
(
	 @FecIni DateTime
	,@FecFin DateTime
	,@PVENT_Id BigInt
...
Select @Orden As Orden
	...
	,IsNull(IsNull(TDPag.TIPOS_DescCorta, 'DB') + ' - ' + Right('0000000000' + RTrim(DPag.DPAGO_Numero), 10)
	 ,TCaj.TIPOS_Desc2 + ' ' + Right('000' + RTRIM(@PVENT_Id), 3) + '-' + Right('0000000' + RTrim(Caj.CAJA_Id), 7)) + '-' + Caj.ENTID_Codigo
	   +
	IsNull((select top 1  
	'' 
	--'-' + cast('00' +  day(IsNull(Rec.RECIB_Fecha,getdate())) as varchar(2))   
	 From  Tesoreria.TESO_Recibos As Rec
...
Select @Orden As Orden
	...
	+
	 	IsNull((select top 1 '-' + Rec.ENTID_Codigo
	 	-- cast('00' +  day(IsNull(Rec.RECIB_Fecha,getdate())) as varchar(2)) 
	 From  Tesoreria.TESO_Recibos As Rec
	--tablas relacionadas a los recibos
	Left Join Tipos As MonInt On MonInt.TIPOS_Codigo = Rec.TIPOS_CodTipoMoneda
