...
DECLARE @PVENT_BaseDatos VARCHAR(25) = 'BDInkaPeru'
DECLARE @PVENT_BDAdmin VARCHAR(25) = 'BDSAdmin'
DECLARE @PVENT_Glosa VARCHAR(200) = 'AREQUIPA - PERU'

DECLARE @PVENT_ZonaContable VARCHAR(3) = 'AQP'
DECLARE @UBIGO_Codigo VARCHAR(10) = '08.01.05' -- CUSCO / CUSCO / SAN SEBASTIAN
DECLARE @EMPRE_Codigo VARCHAR(5) = 'INKAP'
DECLARE @SUCUR_Nombre VARCHAR(50) = 'SUCURSAL CUSCO'
DECLARE @ALMAC_Descripcion VARCHAR(50) = 'ALMACEN PRINCIPAL'
DECLARE @ALMAC_DescCorta VARCHAR(10) = 'PRINCIPAL'

DECLARE @SUCUR_Direccion VARCHAR(60) = 'PROLONGACION AV. DE LA CULTURA 1502 - SAN SEBASTIAN'
DECLARE @PVENT_Direccion VARCHAR(200) = @SUCUR_Direccion
DECLARE @ALMAC_Direccion VARCHAR(120) = @SUCUR_Direccion

DECLARE @Empresaruc VARCHAR(20) = '20601839858'
DECLARE @EmpresaNombre VARCHAR(100) = 'DISTRIBUIDORA INKA PERU S.A.C.'
...
--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::--
IF NOT EXISTS(SELECT * FROM dbo.Entidades WHERE ENTID_NroDocumento = @Empresaruc)

    BEGIN
        DECLARE @ENTID_Id BIGINT = ISNULL((SELECT MAX(ENTID_Id) FROM dbo.Entidades), 0) + 1
    
       INSERT INTO dbo.Entidades
            ( ENTID_Codigo                 , TIPOS_CodTipoDocumento       , UBIGO_Codigo                 , ENTID_Id                     
            , ENTID_TipoEntidadPDT         , ENTID_NroDocumento           , ENTID_RazonSocial            
            , ENTID_NombreComercial        , ENTID_Estado                 , ENTID_Direccion              
            , ENTID_UsrCrea                , ENTID_FecCrea                , ENTID_CodBusqueda            
            )
    VALUES  ( @Empresaruc , -- ENTID_Codigo - CodEntidad
              'DID6' , -- TIPOS_CodTipoDocumento - CodigoTipo
              '08.01.05' , -- UBIGO_Codigo - CodUbigeo
              @ENTID_Id , -- ENTID_Id - Id
              'J' , -- ENTID_TipoEntidadPDT - char(1)
              @Empresaruc , -- ENTID_NroDocumento - NroDocumento
              @EmpresaNombre , -- ENTID_RazonSocial - Nombres
              @EmpresaNombre , -- ENTID_NombreComercial - Nombres
              'A' , -- ENTID_Estado - Estado
              @SUCUR_Direccion , -- ENTID_Direccion - varchar(150)
              'SISTEMAS' , -- ENTID_UsrCrea - Usuario
              GETDATE() , -- ENTID_FecCrea - Fecha
              0  -- ENTID_CodBusqueda - int
            )
