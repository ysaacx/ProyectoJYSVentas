...
IF NOT EXISTS(SELECT * FROM sys.types WHERE name = 'CodDocVentaNew' AND is_table_type = 0 AND SCHEMA_ID('dbo') = schema_id)
    CREATE TYPE [dbo].[CodDocVentaNew] FROM varchar(13) NOT NULL
GO
...
IF NOT EXISTS(SELECT * FROM sys.extended_properties prop INNER JOIN sys.all_columns colum ON colum.column_id = prop.minor_id AND colum.object_id = prop.major_id INNER JOIN sys.all_objects tb ON tb.object_id = colum.object_id 
               WHERE prop.name = 'MS_Description' AND class = 1 AND tb.name = 'VENT_DocsVenta' AND colum.name = 'DOCVE_Codigo')
    EXEC sp_updateextendedproperty 'MS_Description', N'Codigo 
Tipo Documento (2) -> Tipo de Documento
Serie  (4) -> Del Documento de venta
Numero (7) -> Del Documento de venta Rellenado con ceros a la izquierda hasta una longitud (7)
Ejemplo

==  ========
010030001244
  ==', 'schema', 'Ventas', 'table', 'VENT_DocsVenta', 'column', 'DOCVE_Codigo'
GO
...
/*------------------------------------------------------------------------------------------------------------------------------------------*/
ALTER TABLE [Ventas].[VENT_DocsVentaPagos]
DROP CONSTRAINT [PK__VENT_Doc__17A85D1B3732A735]
GO
ALTER TABLE [Ventas].[VENT_DocsVentaPagos]
ALTER COLUMN [DOCVE_Codigo] [CodDocVentaNew] NOT NULL
GO
...
/*------------------------------------------------------------------------------------------------------------------------------------------*/
ALTER TABLE [Ventas].[VENT_DocsRelacion]
DROP CONSTRAINT [PK__VENT_Doc__8731F6583065AE15]
GO
ALTER TABLE [Ventas].[VENT_DocsRelacion]
ALTER COLUMN [DOCVE_Codigo] [CodDocVentaNew] NOT NULL
GO
