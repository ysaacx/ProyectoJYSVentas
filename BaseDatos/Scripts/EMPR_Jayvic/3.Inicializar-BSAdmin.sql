...
DECLARE @EMPR_Codigo VARCHAR(5) = 'JAYVI'
...
--UPDATE dbo.Aplicaciones SET APLI_BaseDatos = NULL WHERE APLI_BaseDatos IS NOT NULL  
 ...
 FROM (  SELECT EMPR_Codigo = @EMPR_Codigo   
              , EMPR_Desc = 'INVERSIONES JAYVIC EIRL.'         
              , EMPR_Direc = 'Cal. Wichaypampa Lt. Z1 - Mza 7 - Vía Principal'
              , EMPR_RUC = '20563926709', EMPR_Servidor = @PVENT_DireccionIP          , EMPR_BaseDatos = 'BDJAYVIC'
...
---DROP TABLE #PMenu
--SELECT * FROM PlantillasMenu WHERE EMPR_Codigo = 'JAYVI'
SELECT DISTINCT EMPR_Codigo FROM PlantillasMenu 
SELECT * INTO #PMenu FROM PlantillasMenu WHERE APLI_Codigo = 'VTA' AND EMPR_Codigo = 'inkap'
