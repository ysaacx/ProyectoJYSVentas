GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LOG_DISTSS_GuiasXDocumento]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[LOG_DISTSS_GuiasXDocumento] 
GO 
-- =============================================
-- Autor - Fecha Crea  : Ysaacx - 14/06/2013
-- Descripcion         : Obtener la Guia de remision por documento de venta
-- Autor - Fecha Modif : 
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[LOG_DISTSS_GuiasXDocumento]
(	
...
WHERE Guia.PVENT_Id = @PVENT_Id
	And Guia.DOCVE_Codigo = @DOCVE_Codigo
Union All
Select Alma.ALMAC_Descripcion As ALMAC_Origen
	, PVenD.PVENT_Descripcion As PVENT_Destino
	, PVenO.PVENT_Descripcion As PVENT_Origen
...
From Logistica.DIST_GuiasRemision As Guia 
	...
	Left Join Tipos As TFDoc On TFDoc.TIPOS_Codigo = IsNull(Ord.TIPOS_CodTipoOrden, 'CPD' + LEFT(Guia.DOCVE_Codigo, 2))
	Left Join Entidades As Us On Us.ENTID_Codigo = Guia.GUIAR_UsrCrea
WHERE Guia.PVENT_Id = @PVENT_Id
	And Ord.DOCVE_Codigo = @DOCVE_Codigo
