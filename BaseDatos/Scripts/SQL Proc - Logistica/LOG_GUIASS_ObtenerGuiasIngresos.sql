GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LOG_GUIASS_ObtenerGuiasIngresos]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[LOG_GUIASS_ObtenerGuiasIngresos] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[LOG_GUIASS_ObtenerGuiasIngresos]
(
...
Where ALMAC_IdDestino = IsNull(@ALMAC_IdDestino, ALMAC_IdDestino)
	...
	--AND  Guia.GUIAR_Estado In ('I', 'C')
	AND Guia.TIPOS_CodMotivoTraslado = @TIPOS_CodMotivoTraslado

If @Todos = 1
	Select * From #Guias
Else
	Select * From #Guias Where GUIAR_Estado In ('I', 'C')
