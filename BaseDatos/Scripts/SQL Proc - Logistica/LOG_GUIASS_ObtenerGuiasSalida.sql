GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LOG_GUIASS_ObtenerGuiasSalida]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[LOG_GUIASS_ObtenerGuiasSalida] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[LOG_GUIASS_ObtenerGuiasSalida]
(
...
Where ALMAC_IdOrigen = IsNull(@ALMAC_IdOrigen, ALMAC_IdOrigen)
	...
	AND PEDID_Codigo Is Null
	
If @Todos = 1
	Select * From #Guias
Else
	Select * From #Guias Where GUIAR_Estado = 'I'
