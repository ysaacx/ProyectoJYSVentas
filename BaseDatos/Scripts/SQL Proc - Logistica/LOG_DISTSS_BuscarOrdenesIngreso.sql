GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LOG_DISTSS_BuscarOrdenesIngreso]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[LOG_DISTSS_BuscarOrdenesIngreso] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[LOG_DISTSS_BuscarOrdenesIngreso]
(
	 @FecIni Datetime
	 ,@FecFin Datetime
	 ,@PVENT_Id BigInt
...
Select Alma.ALMAC_Descripcion As ALMAC_Origen
	...
	,TFDoc.TIPOS_DescCorta + ' ' + Ven.DOCVE_Serie + '-' + RIGHT('0000000' + Rtrim(Ven.DOCVE_Numero), 7) As DocVenta
	,'OC ' + ORDEN_Serie + '-' + Right('0000000' + RTrim(Orden.ORDEN_Numero), 7) As Documento
...
Else
Begin 
	Select * From #Ordenes Where ORDEN_Estado <> 'X'
End

GRANT EXECUTE
ON [dbo].[LOG_DISTSS_BuscarOrdenesIngreso]
TO [Ventas]
