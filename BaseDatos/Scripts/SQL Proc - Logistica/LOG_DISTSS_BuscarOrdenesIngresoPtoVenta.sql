GO 
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[LOG_DISTSS_BuscarOrdenesIngresoPtoVenta]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	DROP PROCEDURE [dbo].[LOG_DISTSS_BuscarOrdenesIngresoPtoVenta] 
GO 
-- =============================================
...
-- Descripcion         : 
-- =============================================
CREATE PROCEDURE [dbo].[LOG_DISTSS_BuscarOrdenesIngresoPtoVenta]
(
	 @FecIni Datetime
	 ,@FecFin Datetime
...
WHERE  
	...
	AND  Orden.PVENT_IdOrigen = @PVENT_IdOrigen
